...
const authGaurd = (...requiredRoles: (keyof typeof USER_ROLE)[]) => {
    return catchAsync(async (req: Request, res: Response, next: NextFunction) => {
      ...
      const decoded = verifyToken(token, config.access_token_secret as string) as JwtPayload;
  
      const { role, email } = decoded;
      // check if the use is exist
      const user = await User.isUserExistsByEmail(email);
      if (!user) {
        throw new AppError(status.UNAUTHORIZED, "Your are not eligible for this operation");
      }
